...
int main(int argc, char **argv)
{
	...
	for (int tx = 0; tx < num_ports; tx++) {
		...
		{
			/* Send data. */
			result = check(sp_blocking_write(tx_port, buff_send_wind, size, timeout));
			...
			result = check(sp_blocking_read(rx_port, buff_rec, size, timeout));
		    ...
		    printf("wind = %.1f ",data_wind);

			/* Send data. */
			result = check(sp_blocking_write(tx_port, buff_send_angle, size, timeout));
		    ...
		    data_angle = (buff_rec[4] + buff_rec[3]*256)/10.0;
		    printf("angle = %.1f\n",data_angle);
		}

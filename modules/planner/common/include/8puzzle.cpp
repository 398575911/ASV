...
class PuzzleState {
 ...
 private:
  ...
  bool LegalMove(TILE *StartTiles, TILE *TargetTiles, int spx, int spy, int tx,
                 int ty);
...
void PuzzleState::PrintNodeInfo() {
  char str[100];
  sprintf(str, "%c %c %c\n%c %c %c\n%c %c %c\n", tiles[0] + '0', tiles[1] + '0',
          tiles[2] + '0', tiles[3] + '0', tiles[4] + '0', tiles[5] + '0',
...
// as if it was on a tile board note : returns false if the board wasn't
// changed, and simply returns the tiles as they were in the target spx and spy
// is the space position while tx and ty is the target move from position

bool PuzzleState::LegalMove(TILE *StartTiles, TILE *TargetTiles, int spx,
                            int spy, int tx, int ty) {
  int t;
...
// function called AddSuccessor to give the successors to the AStar class. The
// A* specific initialisation is done for each node internally, so here you just

...
int main() {
  ...
  auto p_coarse_theta = pathsmoother.coarse_theta();
  ...
  for (std::size_t index = 0; index != p_coarse_forward.size(); ++index) {
    ...
    std::cout << "direction: " << p_coarse_forward[index] << std::endl;
    std::cout << "theta: " << p_coarse_theta[index].at(0) << ", "
              << p_coarse_theta[index].at(1) << std::endl;
